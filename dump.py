...
class EnsemblPlantBioMart(GenericBioMart):
    ...
    def get_latest_mart_version(self):
        ...
        ftp.login()
        release_li = [x for x in ftp.nlst('/pub/fungi') if x.startswith('/pub/fungi/release-')]
    ...
    def select_species(self):
        ...
        try:
            ...
            ftp = FTP(self.__class__.ENSEMBL_FTP_HOST)
            ftp.login()
            species_file = '/pub/fungi/release-%s/mysql/fungi_mart_%s/dataset_names.txt.gz' % (self.release, self.release)
